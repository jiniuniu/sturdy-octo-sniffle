...
def main():
    ...
    result, tracking_data, graph = run_tracked_simulation(
        n=1000,  # 200 nodes for better statistics
        k=6,  # Average degree
    ...
    plt.savefig(
        ...
        / "data"
        / "outputs"
        / "diffusion_sequence_analysis.png",
        dpi=150,
        bbox_inches="tight",
    )
    ...
    first_cat = (
        validation["aware_sequence"][0] if validation["aware_sequence"] else None
    )
    if first_cat == "early_adopters":
        print("✅ Early Adopters became aware first, as expected")
    else:
        print(
            f"⚠️  {first_cat.replace('_', ' ').title()} became aware first, parameters may need adjustment"
        )

    # Check adoption rate trend
    adoption_rates = [
        stats[cat]["adoption_rate"]
        for cat in ["early_adopters", "early_majority", "late_majority", "laggards"]
        if cat in stats
    ]
    ...
    # Check delays
    delays = [
        stats[cat].get("mean_delay", 0) or 0
        for cat in ["early_adopters", "early_majority", "late_majority", "laggards"]
        if cat in stats and stats[cat].get("mean_delay")
    ]
