...
TEST_CONTENTS = [
    ...
    {
        "name": "环保倡议",
        "content": "XX饮料正式启动「绿色瓶盖」计划！即日起，所有产品包装采用100%可回收材料，为地球减负！",
    },
]


def print_initialization_data(personas, users, topology):
    """打印初始化数据"""
    print("\n" + "=" * 50)
    print("初始化数据详情")
    print("=" * 50)
    ...
    for i, persona in enumerate(personas, 1):
        print(f"  {i}. {persona.name} ({persona.percentage:.0%})")
        desc_preview = persona.description[:80] + "..." if len(persona.description) > 80 else persona.description
    ...
    for user in users[:5]:
        ...
        profile_preview = user.profile[:100] + "..." if len(user.profile) > 100 else user.profile
