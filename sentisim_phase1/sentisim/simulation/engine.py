        ...
        for step in range(max_steps):
            ...
            for post in current_wave:
                ...
                if verbose:
                    ...
                    print(
                        f"  - 帖子 {post.post_id[:12]}... ({post_type_str}): "
                        f"受众={len(audience_ids)}, 新触达={post_reached}, 跳过={post_skipped}"
                    )

            if verbose:
                print(
                    f"[Step {step}] 总计: 新触达用户={len(simulation_tasks)}, 跳过已触达={skipped_count}"
                )
            ...
            for i, response in enumerate(responses):
                ...
                step_action_counts[action_name] = (
                    step_action_counts.get(action_name, 0) + 1
                )
                ...
                if new_post:
                    ...
                    if verbose:
                        root_info = (
                            "新传播链"
                            if new_post.post_type == PostType.CREATION
                            else f"继承自 {meta['root_post_id'][:12]}..."
                        )
                        content_preview = (
                            new_post.content[:50] + "..."
                            if len(new_post.content) > 50
                            else new_post.content
                        )
                        print(
                            f"    [新帖子] {new_post.post_id[:12]}... ({new_post.post_type.value}, {root_info}) "
            ...
            if verbose:
                action_summary = ", ".join(
                    f"{k}={v}" for k, v in sorted(step_action_counts.items())
                )
