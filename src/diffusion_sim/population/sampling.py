...
class TraitSampler:
    ...
    @staticmethod
    def sample_normal(
        mean: float, std: float, low: float = 0.0, high: float = 1.0
    ) -> float:
        """
    ...
    @staticmethod
    def sample_traits_from_params(
        trait_params: Dict[str, Dict[str, float]],
    ) -> Dict[str, float]:
        """
...
class PopulationGenerator:
    ...
    def select_initial_adopters(
        self, graph: nx.Graph, n_initial: int, strategy: str = "innovators"
    ) -> List[int]:
        """
