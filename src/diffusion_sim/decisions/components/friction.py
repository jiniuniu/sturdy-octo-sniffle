...
class FrictionModel:
    ...
    def __init__(self, base_friction: float = 0.0, mode: str = "fixed"):
        """
    ...
    def apply(
        self, intention: float, agent_risk_aversion: Optional[float] = None
    ) -> float:
        """
    ...
    def get_friction(self, agent_risk_aversion: Optional[float] = None) -> float:
        ...
        if self.mode == "fixed":
            ...
            base = random.uniform(
                max(0, self.base_friction - 0.1), min(1, self.base_friction + 0.1)
            )
