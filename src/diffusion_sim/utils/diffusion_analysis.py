...
def run_tracked_simulation(
    ...
    sim_config = SimulationConfig(
        max_steps=max_steps, seed=seed, initial_adopters=initial_adopters
    )

    # Run tracked simulation
    engine = TrackedTPBDiffusionEngine(
        graph, sim_config, network_config, decision_model
    )
...
def analyze_diffusion_sequence(tracking_data: Dict) -> Dict:
    ...
    for category in categories:
        ...
        agents_in_cat = [
            aid for aid, info in agent_info.items() if info["category"] == category
        ]
        ...
        stats[category] = {
            ...
            "adoption_rate": (
                len(adopted_times) / len(agents_in_cat) if agents_in_cat else 0
            ),
            "first_adopted": min(adopted_times) if adopted_times else None,
            "median_adopted": (
                float(np.median(adopted_times)) if adopted_times else None
            ),
...
def print_diffusion_report(stats: Dict, validation: Optional[Dict] = None):
    ...
    print("=" * 90)
    print(
        f"{'Category':<20} {'Count':<8} {'Aware%':<10} {'First Aware':<12} {'Median Aware':<12} {'Adoption%':<12}"
    )
    print("-" * 90)

    for cat, data in stats.items():
        first = (
            f"{int(data['first_aware'])}" if data["first_aware"] is not None else "N/A"
        )
        median = (
            f"{data['median_aware']:.1f}" if data["median_aware"] is not None else "N/A"
        )
        print(
            f"{cat.replace('_', ' ').title():<20} {data['count']:>6}  "
    ...
    for cat, data in stats.items():
        mean_delay = (
            f"{data['mean_delay']:.1f}" if data["mean_delay"] is not None else "N/A"
        )
        median_delay = (
            f"{data['median_delay']:.1f}" if data["median_delay"] is not None else "N/A"
        )
        print(
            f"{cat.replace('_', ' ').title():<20} {mean_delay:>13}  {median_delay:>13}"
        )
    ...
    if validation:
        ...
        print(
            f"Expected Sequence: {' → '.join([c.replace('_', ' ').title() for c in validation['expected_aware_sequence']])}"
        )
        ...
        print(
            f"Expected Sequence: {' → '.join([c.replace('_', ' ').title() for c in validation['expected_adopted_sequence']])}"
        )
