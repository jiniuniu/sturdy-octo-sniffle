...
def visualize_diffusion_sequence(
    tracking_data: Dict, result: SimulationResult, figsize=(18, 10)
):
    """
    ...
    if plot_data:
        parts = axes[0, 0].violinplot(
            plot_data, positions=range(len(plot_labels)), showmedians=True
        )
        axes[0, 0].set_xticks(range(len(plot_labels)))
        axes[0, 0].set_xticklabels(
            [c.replace("_", " ").title() for c in plot_labels], fontsize=9
        )
        axes[0, 0].set_ylabel("Awareness Time (step)", fontsize=10)
        axes[0, 0].set_title(
            "Awareness Time Distribution\n(Verify Temporal Sequence)",
            fontsize=11,
            fontweight="bold",
        )
    ...
    if first_aware:
        ...
        axes[0, 1].set_xticklabels(
            [c.replace("_", " ").title() for c in valid_cats], fontsize=9
        )
        axes[0, 1].set_ylabel("First Awareness (step)", fontsize=10)
        axes[0, 1].set_title(
            "First Awareness Time by Category\n(Expected: EA → EM → LM → Laggards)",
            fontsize=11,
            fontweight="bold",
        )
        ...
        for i, v in enumerate(first_aware):
            axes[0, 1].text(
                i, v + 0.5, f"{int(v)}", ha="center", va="bottom", fontweight="bold"
            )
    ...
    for cat in categories:
        ...
        total_in_cat = sum(
            1 for aid, info in agent_info.items() if info["category"] == cat
        )
        ...
        if aware_times and total_in_cat > 0:
            ...
            cumulative_pct = [
                (i + 1) / total_in_cat * 100 for i in range(len(aware_times))
            ]
            axes[0, 2].plot(
                aware_times,
                cumulative_pct,
    ...
    axes[0, 2].set_title(
        "Cumulative Awareness Curves\n(Verify Diffusion Order)",
        fontsize=11,
        fontweight="bold",
    )
    ...
    if plot_adopt_data:
        parts = axes[1, 0].violinplot(
            plot_adopt_data, positions=range(len(plot_adopt_labels)), showmedians=True
        )
        axes[1, 0].set_xticks(range(len(plot_adopt_labels)))
        axes[1, 0].set_xticklabels(
            [c.replace("_", " ").title() for c in plot_adopt_labels], fontsize=9
        )
        axes[1, 0].set_ylabel("Adoption Time (step)", fontsize=10)
        axes[1, 0].set_title(
            "Adoption Time Distribution", fontsize=11, fontweight="bold"
        )
    ...
    if rate_cats:
        ...
        width = 0.35
        axes[1, 1].bar(
            x - width / 2, aware_rates, width, label="Aware", alpha=0.8, color="#3498db"
        )
        axes[1, 1].bar(
            x + width / 2,
            adopt_rates,
        ...
        axes[1, 1].set_xticklabels(
            [c.replace("_", " ").title() for c in rate_cats], fontsize=9
        )
        axes[1, 1].set_ylabel("Rate (%)", fontsize=10)
        axes[1, 1].set_title(
            "Awareness vs Adoption Rate\n(Expected: Decreasing)",
            fontsize=11,
            fontweight="bold",
        )
    ...
    for cat in categories:
        ...
        for aid, info in agent_info.items():
            if (
                info["category"] == cat
                and info["aware_at"] is not None
                and info["adopted_at"] is not None
            ):
    ...
    if plot_delays:
        ...
        axes[1, 2].set_title(
            "Aware → Adopt Delay\n(Decision Speed, Expected: Increasing)",
            fontsize=11,
            fontweight="bold",
        )
...
def plot_category_comparison(
    tracking_data: Dict, result: SimulationResult, figsize=(12, 5)
):
    """
    ...
    for cat in categories:
        agents_in_cat = [
            aid for aid, info in agent_info.items() if info["category"] == cat
        ]
    ...
    axes[0].set_xticklabels(
        [m["category"].replace("_", " ").title() for m in metrics], fontsize=9
    )
