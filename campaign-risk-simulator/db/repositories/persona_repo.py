...
async def insert_placeholders_for_study(study_id: str, positions: list[dict]):
    db = get_db()
    docs = [
        {
            "study_id": study_id,
...
async def update_for_study(study_id: str, persona_id: str, output: PersonaOutput):
    db = get_db()
    await db.personas.update_one(
        {"study_id": study_id, "persona_id": persona_id},
        {
            "$set": {
...
async def get_by_study(study_id: str) -> list[dict]:
    db = get_db()
    cursor = db.personas.find({"study_id": study_id})
    return [doc async for doc in cursor]


async def count_by_study(study_id: str) -> int:
    db = get_db()
    return await db.personas.count_documents({"study_id": study_id})


async def count_completed_by_study(study_id: str) -> int:
    db = get_db()
    return await db.personas.count_documents({"study_id": study_id, "completed": True})
