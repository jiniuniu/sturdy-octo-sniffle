...
function goStep(n) {
  ...
  document.getElementById("sim-day").style.display =
    n === 3 ? "inline" : "none";
  ...
  if (n === 3 && !ctx.isSnapshot) {
    ...
    document.getElementById("feed-list").innerHTML =
      '<div style="color:#444;font-size:12px;padding:16px;text-align:center" data-placeholder>模拟开始后显示评论</div>';
...
async function loadWorldList() {
  ...
  try {
    ...
    if (!data.worlds.length) {
      listEl.innerHTML =
        '<div class="world-empty">暂无社区，请运行脚本创建</div>';
      return;
    }
    listEl.innerHTML = data.worlds
      .map((w) => {
    ...
    listEl.innerHTML =
      '<div class="world-empty" style="color:#ff4444">加载失败</div>';
  }
}

async function selectWorld(worldId, el) {
  document
    .querySelectorAll(".world-item")
...
async function loadWorldDetail(worldId) {
  ...
  document.getElementById("detail-dims").textContent = (ctx.dimensions || [])
    .map((d) => d.name)
    .join(" · ");

  const listEl = document.getElementById("sim-history-list");
  listEl.innerHTML =
    '<div style="color:#444;font-size:12px;padding:8px 0">加载中...</div>';
  try {
    const res = await fetch(`/worlds/${worldId}/simulations`);
    const data = await res.json();
    if (!data.simulations.length) {
      listEl.innerHTML =
        '<div style="color:#444;font-size:12px;padding:8px 0">暂无历史模拟</div>';
      return;
    }
    listEl.innerHTML = data.simulations
      .map((s) => {
    ...
    listEl.innerHTML =
      '<div style="color:#ff4444;font-size:12px;padding:8px 0">加载失败</div>';
  }
}
...
const reachChart = new Chart(chartCtx, {
  ...
  options: {
    ...
    plugins: {
      legend: { labels: { color: "#666", font: { size: 10 }, boxWidth: 10 } },
    },
    scales: {
      x: { display: false },
      y: {
...
const simulation = d3
  ...
  .force(
    ...
    d3
      .forceManyBody()
      .strength((d) =>
        d.tier === "kol" ? -600 : d.tier === "koc" ? -300 : -120,
      ),
  )
  .force(
    "collide",
    d3.forceCollide((d) => (tierRadius[d.tier] || 3) + 6),
  )
...
function renderGraphInit(nodes, edges) {
  ...
  gLinks
    .selectAll("line")
    .data(edgeData, (d) => `${d.source}__${d.target}`)
    .enter()
    .append("line")
...
function appendLog(data) {
  ...
  const tier =
    data.agent_tier === "kol" ? "🟠" : data.agent_tier === "koc" ? "🔵" : "·";
...
function fillComment(data) {
  ...
  const cards = [
    ...list.querySelectorAll(`.comment-card[data-agent-id="${data.agent_id}"]`),
  ];
  const card = cards
    .reverse()
    .find((c) => c.querySelector(".comment-body")?.textContent === "生成中...");
  if (card) {
...
function setRunning(running) {
  document.getElementById("btn-stop").style.display = running
    ? "inline-block"
    : "none";
...
function startSim(simId) {
  ...
  // 重置
  graphState.nodeIndex = {};
  graphState.linkIndex = {};
  ...
  es.onmessage = (e) => {
    ...
    if (data.type === "agent_reacted") {
      ...
      document.getElementById("sim-day").textContent =
        `Day ${data.sim_time.toFixed(2)}`;

      if (data.highlight_edge) {
        highlightEdge(
          data.highlight_edge.source,
          data.highlight_edge.target,
          data.action,
        );
      }

      if (data.seed_stats) {
        seedStats = data.seed_stats;
        updateSeedStats();
      }
      updateMetrics(data.metrics);
      appendLog(data);
    ...
    if (data.type === "brand_reply") {
      appendBrandReply(data);
      appendBrandLog(data);
    }
