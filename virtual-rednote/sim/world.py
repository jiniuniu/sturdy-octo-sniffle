...
TIER_CONFIG = {
    AgentTier.KOL: {
        ...
        "sharing":        (0.6, 1.0),
        "vec_alpha": 5.0, "vec_beta": 2.0,   # 偏高端，体现专业偏好
    },
    AgentTier.KOC: {
        "activity":       (0.4, 0.8),
        "expressiveness": (0.4, 0.8),
        "sharing":        (0.3, 0.7),
        "vec_alpha": 3.0, "vec_beta": 2.0,   # 有偏好但不极端
    },
    AgentTier.NORMAL: {
        "activity":       (0.1, 0.5),
        "expressiveness": (0.1, 0.4),
        "sharing":        (0.05, 0.3),
        "vec_alpha": 2.0, "vec_beta": 2.0,   # 均匀分布，无强烈偏好
    },
}
...
def generate_agents(
    ...
    for tier, count in [(AgentTier.KOL, n_kol), (AgentTier.KOC, n_koc), (AgentTier.NORMAL, n_normal)]:
...
def generate_social_graph(agents: list[Agent], seed: int = 42) -> nx.DiGraph:
    ...
    for normal in normals:
        for kol in rng.choice(kols, size=min(rng.integers(1, 4), len(kols)), replace=False):
            add_follow(normal, kol)
        for koc in rng.choice(kocs, size=min(rng.integers(2, 6), len(kocs)), replace=False):
            add_follow(normal, koc)
        peers = [a for a in normals if a.id != normal.id]
        if peers:
            for peer in rng.choice(peers, size=min(rng.integers(0, 4), len(peers)), replace=False):
